...
class PointerNet(object):
  ...
  def __init__(self, batch_size=128, max_input_sequence_len=5, max_output_sequence_len=7, 
    ...
    # Create placeholders
    self.inputs = tf.placeholder(tf.float32, shape=[self.batch_size,self.max_input_sequence_len,969], name="inputs")
    ...
    special_token_embedding = tf.get_variable("special_token_embedding", [3,969], tf.float32, tf.contrib.layers.xavier_initializer())
    # Embedding_table
